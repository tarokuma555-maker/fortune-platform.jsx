...
import { ANIMALS, ANIMAL_60, ANIMAL_COLORS } from "@/data/animals";
...
export function calcAnimal(y, m, d) {
  ...
  // キャラナンバー（1〜60）
  let charNum = (jdn + 18) % 60;
  if (charNum === 0) charNum = 60;
  const entry = ANIMAL_60[charNum - 1];
  const base = ANIMALS[entry.animalIdx];
  const colorInfo = ANIMAL_COLORS[entry.color];
  return {
    ...base,
    color: entry.color,
    colorCode: colorInfo.colorCode,
    colorTrait: colorInfo.trait,
    fullName: `${entry.color}の${base.name}`,
...
export function buildFortuneSummary(results) {
  ...
  lines.push(`【動物占い】${r.animal.fullName}（No.${r.animal.characterNumber}）。${r.animal.personality}`);
...
export function buildFortuneSummaryWithAI(results) {
  ...
  const sections = [
    ...
    { key: "animal", label: "動物占い", base: `${r.animal.fullName}（No.${r.animal.characterNumber}）。${r.animal.personality}` },
